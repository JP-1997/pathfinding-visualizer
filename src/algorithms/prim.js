const prim = (grid, rows, columns) => {
  ...
  while (Object.keys(frontier).length) {
    ...
    const { row, column } = frontier[randFKey];
    let neighbours = {};
    f.every((i, index) => {
      const r = row + i[0];
      const c = column + i[1];
    ...
    removedWalls.push(grid[randNeighbour.row][randNeighbour.column]);
    removedWalls.push(grid[frontier[randFKey].row][frontier[randFKey].column]);
    f.forEach(i => {
